      ...
      <div className="container flex justify-center items-center h-16">
        ...
        >
          <svg
            ...
            className={`h-6 w-6 transition duration-500 transform-gpu ${menu ? 'text-primary rotate-180' : 'text-white rotate-0'}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
        ...
        <nav className={`fixed z-10 inset-y-0 right-full w-80 pt-16 bg-white text-primary transition duration-500 transform-gpu will-change-transform ${menu ? 'translate-x-full' : 'translate-x-0'}`}>
          <ul className="flex flex-col">
            {categories?.edges?.map(({ node }) => (
              <li key={node.id}>
                <Link href={`/${node.slug}`}>
                  <a className="flex items-center px-5 py-2 font-semibold text-lg hover:bg-primary hover:text-white">
                    {node.name}
                  </a>
                </Link>
              </li>
